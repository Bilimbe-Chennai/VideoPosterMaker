...
router.post("/videophoto", async (req, res) => {
  try {
    ...
    req.on("end", async () => {
      ...
      await media.save();
       const downloadUrl = `http://localhost:5000/api/upload/file/${media.mergedVideoId}?download=true`;
...
router.post(
  ...
  async (req, res) => {
    ...
    req.on("end", async () => {
      ...
      await media.save();
     const downloadUrl = `http://localhost:5000/api/upload/file/${media.posterVideoId}?download=true`;
      const qrCodeData = await QRCode.toDataURL(downloadUrl);
