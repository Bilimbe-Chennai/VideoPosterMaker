...
const TemplateManager = () => {
  ...
  useEffect(() => {
    // Fallback for Super Admin: Might not have 'admin' in localStorage, but 'user'. 
    // Or if super admin is creating templates, they might not need 'templateCount' limit, but for form consistency let's default.
    const adminData = localStorage.getItem('user') || localStorage.getItem('admin');
    if (adminData) {
      try {
        const user = JSON.parse(adminData);
        setTemplateCount(user.templateCount || 3); // Higher default for SuperAdmin
        setUserAccess(user.accessType || []);
        setAdminInfo({
          id: user._id || user.id || '',
          branchid: user.branchid || '' // SuperAdmin might not have branchid, handled in backend or optional
        });
      } catch (e) {
  ...
  const saveTemplates = (newList) => {
    ...
    // localStorage.setItem('photo_templates', JSON.stringify(newList)); // Optional persistence
  };
          ...
          {filteredTemplates.map(tmpl => (
            <TemplateCard key={tmpl.id} $selected={selectedIds.includes(tmpl.id)}>
              <TemplatePreview $active={tmpl.status === 'active'} onClick={() => handleEdit(tmpl)}>
                ...
                >
                  {selectedIds.includes(tmpl.id) && <ArrowUpRight size={12} />}
                </SelectionCircle>
              </TemplatePreview>
              <TemplateInfo>
                <TemplateTitle>
